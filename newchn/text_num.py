...
def write_result(doc, para_no, alab_chn_list, crt_chn_list, cor_ornot):
    ...
    #with open('./test_result/alab_chn_cpy-wxht1.txt', 'a') as f:
    with open('./test_result/alab_chn_cpy-test.txt', 'a') as f:
...
def text_num_match(doc_path):
    doc_path = norm_filename(doc_path)
    for doc in sorted(doc_path):
        ...
        for para in file.paragraphs: # read the content from word documents
            ...
            alab_num_iter = re.finditer(r'([\d.]+[万]*元)', white_text)
            ...
            chn_num_iter = re.finditer(r'(?<!\d)[壹贰叁肆伍陆柒捌玖万仟千佰百拾亿零一二三四五六七八九十/]+元+[零壹贰叁肆伍陆柒捌玖一二三四五六七八九十角分]*', white_text)

            for item in chn_num_iter:
                chn_index.append(item.span()[0])
                chn_num.append(item.group())
            #print(chn_num)
            for i in chn_num:  # remove items that contain /
                if i.find('/')!=-1:
                    chn_num.remove(i)
            ...
            for num in alab_num_1:
                ...
                num_a_chn = '' #小数点后数字的中文
                if len(num_a)!=0:
                    if num_a[0]!='0' or num_a[1]!='0':
                        num_a_chn += '元'
                    if num_a[0]!='0':
                        num_a_chn += dict_num[int(num_a[0])]+'角'
                    elif num_a[0]=='0' and num_a[1]!='0':
                        num_a_chn += dict_num[int(num_a[0])]
                    if len(num_a)==2 and num_a[1]!='0':
